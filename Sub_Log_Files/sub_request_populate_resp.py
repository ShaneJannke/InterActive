...
def sub_request_populate_resp(Main_Tree,Sub_clicked,Report_Text,Sub_Options_V):
    ...
    '''
    6/7/22 SJ
    Get the selected Sub Request status to search by alongside the responsible party
    '''
    ...
    try:
        result = Sub_clicked.get()
        c.execute(EXE_STRING_user,result,status)            
        data = c.fetchall()
