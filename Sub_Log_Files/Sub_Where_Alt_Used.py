...
def Sub_Where_Alt_Used(Main_Tree,Report_Text,Book):
    ...
    Book.update()
    PN = simpledialog.askstring("PN", "Enter the Alt Part Number")
    ...
    if PN == None:
        ...
        EXE_STRING ="EXEC Interactive.dbo.Sub_Where_Alt_Used @alt=?"
        ...
        try:
            ...
            data = c.fetchall()
            if not data:
                messagebox.showerror("Where Alt Used", "No data found.")
            else:
                '''
                Uses Main_Tree_10 to show different data
                '''
                Main_Trees.Clear_Main_Tree(Main_Tree)
                Main_Trees.Main_Tree_10(Main_Tree)
                ...
                Report_Text.insert(END, "Viewing Parts Where Alt: " + PN + " is used" "\n")
