...
#New sub request button most coppied from Sub Log
def TM_New_Request(Main_Tree,Sub_Request_V,Report_Text,Book):
    ...
    Generate = "EXEC Interactive.dbo.TM_Generate_Request_ID"
    Remove_Iniated_1 = "EXEC Interactive.dbo.TM_Remove_Initated_ID_1" #This removes all intiated from the log
    ...
    try:  
        ...
        '''
        get_data = "SELECT MAX(REQUEST_ID) FROM [Interactive].[dbo].[TM_REQUEST_ID_GENERATOR] "
        c.execute(get_data)
        data1 = c.fetchall()
        ...
        exe_String = "EXEC Interactive.dbo.TM_By_ID @id=?"
